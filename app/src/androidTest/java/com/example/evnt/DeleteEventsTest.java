...
public class DeleteEventsTest {
    ...
    @Test
    public void delete_an_event() throws Exception {
        /* My Events Fragment */
        onView(
                allOf(
        ...
        System.out.println("We see a delete button, in hosting tab, we click it");

        onView(withIndex(withId(R.id.delete_button),0)).perform(click());


        onView(withText("Delete Event")).check(matches(isDisplayed()));
        System.out.println("Delete button press prompts us to double check our action");
